...
const initCore = () => {
  ...
  // Tema değişikliği için izleyici
  watch(
    () => configStore.theme,
    (newTheme) => {
      localStorage.setItem('app-theme', newTheme) // Yeni tema değerini localStorage'a kaydet
    },
    { immediate: true }
  )

  // Sayfa yüklendiğinde kaydedilen temayı yükle
